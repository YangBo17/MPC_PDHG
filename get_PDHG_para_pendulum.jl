...
Lu[:,3*(d+1)+1:4*(d+1),1]=D
Lu[:,:,1]-=kron((pinv(H)*Gflat)[1,:]',Id1)

Lu[:,3*(d+1)+1:4*(d+1),2]=-D
Lu[:,:,2]+=kron((pinv(H)*Gflat)[1,:]',Id1)
...
for l in 1:N # 对应r全为0
    ...
    h[r_seg_ind+d+2:r_seg_ind+2*(d+1),c_seg_ind+d+2:c_seg_ind+2*(d+1)]=-D
    h[r_seg_ind+d+2:r_seg_ind+2*(d+1),c_seg_ind+2*(d+1)+1:c_seg_ind+3*(d+1)]=Matrix(1.0I,d+1,d+1)
    h[r_seg_ind+2*(d+1)+1:r_seg_ind+3*(d+1),c_seg_ind+2*(d+1)+1:c_seg_ind+3*(d+1)]=-D
    h[r_seg_ind+2*(d+1)+1:r_seg_ind+3*(d+1),c_seg_ind+3*(d+1)+1:c_seg_ind+4*(d+1)]=Matrix(1.0I,d+1,d+1)
end
############################### 段间连续性要求有 （N-1)*n 个##################################
basis=zeros(d+1)
for i in 1:d+1
...
I_objn=[kron(Matrix(1.0I,N,N), diagm([500;1;1;1])) zeros(n*N,m*N);zeros(m*N,n*N) Matrix(1.0I,m*N,m*N)]
P=kron(I_objn,Qx)
